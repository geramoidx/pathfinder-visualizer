window.onload = () => {
  ...
  for (let i = 0; i < speedbuttons.length; i++) {
    speedbuttons[i].onclick = () => {
      ...
      document.getElementsByClassName("spd")[0].textContent = speedbuttons[
        i
      ].getAttribute("value");
    };
  }
  ...
  bombBtn.addEventListener("click", (event) => {
    ...
    if (addbomb) {
      ...
      BombDragNode = new BombNode(BombCenterX, BombCenterY)
        .show()
        .animateBomb();
      UpdateBombCanvas();
    } else {
    ...
    function CalculateGridNodes() {
      ...
      if (true) {
        ...
        if (VERTICAL) {
          ...
          keyindex =
            Math.floor(Math.random() * (xrefend - xrefstart)) + xrefstart;

          keyindex = keyindex % 2 == 0 ? keyindex : keyindex + 1;

          let rand =
            Math.floor(Math.random() * (yrefend - yrefstart)) + yrefstart;
  ...
  function GreedyBestSearch(bomb = false, alpha = false) {
    ...
    Enqueueing.unshift([
      [StartingNode],
      ManhattanDistance(StartingNode, TargetNode),
    ]);
                    ...
                    if (SOLVED) {
                      if (EdgeNode !== undefined) {
                        ...
                        if (BidirectionalIndex.indexOf(NodeMap) == -1) {
                          ...
                          for (let x = 0; x < ShiftedBidirections.length; x++) {
